...
$(function () {
    ...
    console.log("jqiery is up and running...");
});
...
function getGradeScale() {
    $.ajax({
        ...
        processData: false,
        async: false,
        url: "/ws/xte/section/grade_scale?section_ids=" + request.psData.section_dcid,
...
function getStandards2() {
    var stds = $.map(request.psData.standards, function (v) {
        return parseInt(v.standard_dcid);
    });
    $.ajax({
        type: "GET",
        dataType: "json",
        ...
        processData: false,
        async: false,
        url: "/ws/xte/standard/list/" + stds.join(','),
        success: function (response) {
...
function getStandards() {
    $.ajax({
        ...
        processData: false,
        async: false,
        url: "/ws/xte/section/standard?include_comment_only=false&include_nonassignment=false&section_ids=" + request.psData.section_dcid,
        success: function (response) {
            standards = [];
            $.each(response, function (i, v) {
                standards.push(v);
                if (v._childstandards) {
                    $.each(v._childstandards, function (i2, v2) {
                        standards.push(v2);
                    });
                }
            });
...
function getAssignment() {
    $.ajax({
        ...
        processData: false,
        async: false,
        url: "/ws/xte/section/assignment?users_dcid=" + request.psData.teacher_dcid + "&section_ids=" + request.psData.section_dcid,
        success: function (assignments) {
            debugger;
            //var assignments = JSON.parse(response);
            var list = $.grep(assignments, function (v) {
                return v.assignmentid == request.assignment.assignmentid;
            });
            if (list.length > 0) {
                request.assignment = list[0];
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            $(".card-footer").show();
...
function syncScores() {
    ...
    xdLocalStorage.getItem("fpk12-bridge", function (data) {
        request = JSON.parse(data.value);
        getGradeScale();
        getStandards();
        getAssignment();

        var stdsAdded = $.map(request.assignment._assignmentstandardassociations, function (v) {
            return v.standardid;
        });

        var stds = $.map(request.psData.standards, function (v) {
            return stdsAdded.indexOf(parseInt(v.standard_dcid)) == -1 ? parseInt(v.standard_dcid) : null;
        });

        //check standards
        var count = 0;
        $.each(standards, function (i, v) {
            if (stds.indexOf(v.standardid) > -1) {
                count++;
                request.assignment._assignmentstandardassociations.push({ "standardid": v.standardid, "iscountedinstandardgrade": true, assignmentid: request.assignment.assignmentid });
            }
        });
        ...
        if (count > 0) {
            $.ajax({
                ...
                processData: false,
                async: false,
                url: "/ws/xte/section/assignment/" + request.assignment.assignmentid,
                data: JSON.stringify(request.assignment),
                success: function (response) {
                    getAssignment();
                },
                error: function (jqXHR, textStatus, errorThrown) {
        ...
        var scores = [];
        $.each(request.psData.studentScores, function (i, v) {
            var score = $.grep(request.scores, function (v1) {
                return v1.studentsdcid == v.student_dcid && v1.scorepoints != undefined;
            });
            if (score.length > 0) {
                score[0].actualscoreentered = v.score;
                score[0].scorepoints = v.score;
                scores.push(score[0]);
            } else {
                //if (v.score > 0)
                //{
                scores.push({
                    "studentsdcid": v.student_dcid,
                    "islate": false,
                    "iscollected": false,
                    "isexempt": false,
                    "ismissing": false,
                    "isabsent": false,
                    "isincomplete": false,
                    "actualscoreentered": v.score,
                    "actualscorekind": "REAL_SCORE",
                    "actualscoregradescaledcid": gradescale._gradescales[0].dcid,
                    "_assignmentsection": {
                        "assignmentsectionid": request.assignment._assignmentsections[0].assignmentsectionid,
                        "sectionsdcid": request.assignment._assignmentsections[0].sectionsdcid,
                        "_assignment": {
        ...
        var tempStds = {};
        $.each(request.assignment._assignmentstandardassociations, function (i, v) {
            tempStds[v.standardid] = v.assignmentstandardassocid;
        })

        var standardScores = [];
        $.each(request.psData.standardScores, function (i, v) {
            var score = $.grep(request.standardScores, function (v1) {
                return v1.standardid == v.standard_dcid && v1.studentsdcid == v.student_dcid && v1.scorepoints != undefined;
            });
            if (score.length > 0) {
                score[0].actualscoreentered = v.score;
                score[0].scorepoints = v.score;
                standardScores.push(score[0]);
            } else {
                if (v.score > 0) {
                    var standard = $.map(standards, function (x) {
                        if (x.standardid == v.standard_dcid) {
                            return x
                        }
                    });
                    if (standard.length > 0 && tempStds[v.standard_dcid] != undefined) {
                        standardScores.push({
                            "_assignmentstandardassociation": {
                                "iscountedinstandardgrade": true,
                                "_name": "assignmentstandardassoc",
                                "standardid": v.standard_dcid,
                                "assignmentid": request.assignment.assignmentid,
                                "assignmentstandardassocid": tempStds[v.standard_dcid]
                            },
                            "standardid": v.standard_dcid,
                            "studentsdcid": v.student_dcid,
                            ...
                            "actualscoreentered": v.score,
                            "actualscorekind": "REAL_SCORE",
                            "actualscoregradescaledcid": standard[0].gradescaleitemdcid,
                            "_assignmentsection": {
                                "assignmentsectionid": request.assignment._assignmentsections[0].assignmentsectionid,
                                "sectionsdcid": request.assignment._assignmentsections[0].sectionsdcid,
                                "_assignment": {
        ...
        $.ajax({
            ...
            processData: false,
            async: false,
            url: "/ws/xte/score?push_assignment_scores=false&status=A,I,P",
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                debugger;
                $(".alert .alert-danger").text(errorThrown);
                $(".card-footer").show();
                $(".alert-danger").show();
            }
        });
